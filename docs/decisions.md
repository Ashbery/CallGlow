...
6. （v1.0.3b）設定頁右滑關閉整個移除——16:57:34 實測 swipe_dismiss src=settings 證明 ScrollView
   滾動會誤觸；來電畫面保留嚴格橫向判定並新增「CALLING 中忽略系統返回」（onBackPressed 覆寫）。
7. （v1.0.3c 使用者回饋）系統右滑返回在此錶不可用 → **恢復設定頁右滑返回**，但以嚴格橫向判定
   （跟手 dx≥40 且 |dy|≤dx；關閉 dx≥100 且 |dy|≤dx×0.25）＋關閉事件記錄 dx/dy；來電畫面門檻同步
   0.3→0.25。若仍誤觸，log 的 dx/dy 可直接定位真實軌跡。
...
10. （v1.0.3f 使用者實測：白名單後系統右滑返回仍不可用）**此錶無系統邊緣返回手勢**（HeyTap 手勢僅
    快捷啟動用途）→ 自訂滑動**加回**，但限定「**左緣 100px 內起手**＋嚴格水平（|dy|≤dx×0.25）」。
11. （v1.0.3g）滑動關閉一度全移除＋設定頁加返回按鈕（使用者否決按鈕）。
12. （v1.0.3i 終局定案）診斷實測（19:06 觸控座標 45 筆）：**觸控軸正常**（垂直滑=dy 大、水平滑=dx 大），
    使用者自然返回手勢＝**左緣起手**往右滑（實測 15+ 次，起手 x=10~85）；先前「誤判」＝中間起手
    的斜向滑動（dx 大、dy 小於閾值，例如 dx=170/dy=23 被舊規則當水平）。最終規則：
    設定頁「左緣 110px 內起手＋dx≥100＋|dy|≤dx×0.4」才返回（等同系統返回區設計，容許 ±21°）；
    中間起手任何滑動永不觸發；**來電畫面不支援滑動關閉**（CALLING 忽略返回、未接/斷線 8s 自動關）；
    返回按鈕移除。
13. （v1.0.3j 使用者裁決）「想返回」與「誤觸」的右滑物理軌跡重疊（19:06 診斷 45 筆、19:09 誤觸 2 筆
    對照：角度/起手位置皆同範圍）→ 加**甩動速度門檻**：左緣起手＋水平＋**速度 ≥1.2px/ms**
    （≈300px 在 250ms 內甩完）才返回；慢速滑動永不觸發。log 記錄 vel。
14. （v1.0.3k 使用者要求）**來電畫面與未接畫面恢復滑動關閉**（先前 v1.0.3g 誤刪）：同一套
    「左緣 110px＋dx≥100＋|dy|≤0.4dx＋甩動 ≥1.2px/ms」規則；CALLING＝本地停震關畫面
    （等同 endCall(false)，不送 BLE）、未接/斷線＝關閉；CALLING 中系統返回仍忽略。
