...
- 手勢（v1.0.3e 根因修正）：自訂滑動偵測**全部移除**。系統 HeyTap 手勢需白名單
  （heytap_gesture_enable_app_list；install.bat 自動加入 com.linewatch.watch）——
  返回/關閉統一由系統手勢處理；來電畫面保留「CALLING 中忽略系統返回」（onBackPressed 覆寫）＋
  只隱藏狀態列（保留手勢區）。
