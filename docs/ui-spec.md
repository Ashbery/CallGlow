...
- 設定頁右滑返回（v1.0.3c **恢復**）：系統右滑返回在此錶不可用，恢復自訂右滑返回，但改嚴格橫向判定
  （跟手門檻 dx≥40 且 |dy|≤dx；關閉門檻 dx≥100 且 |dy|≤dx×0.25）＋關閉事件記錄 dx/dy 座標；
  往下滑/對角下滑不觸發。來電畫面同規則（dx≥80、|dy|≤dx×0.25）＋CALLING 中忽略系統返回。
