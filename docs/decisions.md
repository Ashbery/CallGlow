...
11. （v1.0.3g 終局：實測 log 證明誤判者皆為「真·水平右滑」（dx 140~383、|dy|≤50）——人手垂直
    漂移無法根除，任何滑動偵測都會誤判）→ **全部滑動關閉移除**：設定頁新增「‹ 返回」按鈕＋實體
    按鍵（KEYCODE_BACK）；來電畫面 CALLING 中忽略系統返回（測試模式允許返回關閉）、未接/斷線
    8s 自動關；**零滑動＝零誤判**。
