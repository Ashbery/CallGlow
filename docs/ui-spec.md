...
- 手勢（v1.0.3k）：設定頁返回、**來電畫面、未接畫面**統一規則＝左緣 110px 內起手 ＋ dx≥100 ＋
  |dy|≤dx×0.4 ＋ 甩動速度 ≥1.2px/ms（慢速滑動永不觸發）；中間起手任何滑動永不觸發。
  來電畫面滑動＝本地停震關畫面（等同 endCall(false)，不送 BLE）；未接/斷線＝關閉（8s 自動關保留）；
  CALLING 中系統返回仍忽略。
