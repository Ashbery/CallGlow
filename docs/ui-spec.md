...
- F 3D 銀河星盤（D17.5，使用者提議 3D）：EdgeHaloView 星帶改為傾斜 32° 的銀河盤面——
  每顆星 3D 座標（θ、半徑 0.90–0.99、厚度 z 抖動 ±5%）投影到 2D（純 Canvas 數學，無 GPU 依賴）：
  近星放大至 1.3×＋變亮、遠星縮至 0.6×＋變暗、按深度排序繪製（前後遮蔽）→ 立體景深；
  盤面 48s 繞軸自轉、3.5s 呼吸。星群維持 D17.4（微星 250／亮星 28／高亮星 8）。
