...
object PulsarPresets {
    ...
    val ITEMS: List<Item> = listOf(
        ...
        Item("chime", "鐘聲", "Chime", 380),
        Item("bellToll", "鐘鳴", "Bell Toll", 399),
        Item("barrage", "彈幕", "Barrage", 309),
        Item("catPaw", "貓掌", "Cat Paw", 75),
        Item("dewdrop", "露珠", "Dewdrop", 210),
    ...
    fun playOnce(pulsar: Pulsar, key: String): Boolean = when (key) {
        ...
        "clamor" -> play(pulsar.getPresets()::clamor)
        ...
        "chime" -> play(pulsar.getPresets()::chime)
        "bellToll" -> play(pulsar.getPresets()::bellToll)
        "barrage" -> play(pulsar.getPresets()::barrage)
        "catPaw" -> play(pulsar.getPresets()::catPaw)
        "dewdrop" -> play(pulsar.getPresets()::dewdrop)
