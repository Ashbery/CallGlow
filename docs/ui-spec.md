...
- D17.8 文字配色/特效：CALLING 標題「LINE 來電/視訊來電」改 LINE 品牌綠（與白色名字區分）；
  副標「● 震動提醒中」改星雲青。D17.9/D17.10（使用者：閃爍、心跳、脈動都醜）→
  「柔光呼吸」：文字恆定不閃不跳，僅 shadow 光暈（星雲青 setShadowLayer）隨節拍
  擴散 6dp（240ms）／收縮 2dp（320ms）；Activity 與 overlay 同步。
...
- 上限（v1.0.2 使用者裁決）：總容量 ≤500MB（每張 ≤12KB，≈4 萬人）、索引 ≤50,000 筆安全上限；LRU（最久未用）淘汰；
  每張 JPEG q90 重壓寫入；同名字重複來電覆寫同一格（刷新時間，不佔新格）。
- 震動模式（v1.0.3）：原「系統預定義效果 4 種」由 **Pulsar（com.swmansion:pulsar 1.3.0）15 種預設**取代；
  Settings「震動模式」＝自訂節奏＋15 種 Pulsar 預設（點選即預覽）；來電時按「預設時長＋150ms」循環重發。
  v1.0.3b：實測 15 種全數送達震動服務（dumpsys vibrator 48 筆波形佐證）；短/低振幅者手錶馬達幾乎無感，
  → 換入 warDrum/tickTock/woodpecker/hammer（移除 catPaw/dewdrop/chime/clamor）。
  v1.0.3d：預覽加防切斷（同預設播放中＋200ms 內重複點擊忽略——快速連點不再砍掉長預設）。
- 震動節奏列（急促/適中/長震）v1.0.3d **移除**（模式＋強度即足夠）；「自訂節奏」選項一併移除，
  模式＝15 種 Pulsar 預設（預設 alarm）；強度設定作用於未接震動（Pulsar 預設內建振幅）。
- 手勢（v1.0.3f 定案）：此錶**無系統邊緣返回手勢**（實測白名單無效）→ 自訂「左緣手勢區」：
  起手點須在左緣 100px 內（≈屏寬 21%，等同手機系統返回區）＋嚴格水平（跟手 |dy|≤dx、
  關閉 dx≥100 且 |dy|≤dx×0.25）；中間下滑/右上滑/任意位置滑動不觸發。
  設定頁＝返回；來電畫面＝本地停震關畫面（CALLING）／關閉（未接/斷線）；CALLING 中
  onBackPressed 忽略系統返回維持。
